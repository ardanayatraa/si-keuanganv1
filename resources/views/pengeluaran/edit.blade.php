                ...
                <div class="space-y-4">
                        ...
                        <select id="id_pengguna" name="id_pengguna"
                            ...
                            @foreach (\App\Models\Pengguna::all() as $u)
                                <option value="{{ $u->id_pengguna }}"
                                    {{ old('id_pengguna', $pengeluaran->id_pengguna) == $u->id_pengguna ? 'selected' : '' }}>
                                    {{ $u->username }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea id="deskripsi" name="deskripsi" rows="3"
                            class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-800 dark:text-white">{{ old('deskripsi', $pengeluaran->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
