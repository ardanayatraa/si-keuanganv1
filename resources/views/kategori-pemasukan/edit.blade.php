<x-app-layout>
    ...
    @php
        ...
        $currentIcon = old('icon', $kategoriPemasukan->icon);
    @endphp
    ...
    <div class="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        <form
            action="{{ route('kategori.update', [
                'id' => $kategoriPemasukan->id_kategori_pemasukan,
                'type' => $type,
            ]) }}"
            method="POST">
            @csrf
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <input id="nama_kategori" name="nama_kategori" type="text"
                        value="{{ old('nama_kategori', $kategoriPemasukan->nama_kategori) }}"
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="3"
                        class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm
                                     focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-800 dark:text-white"
                        placeholder="Opsional…">{{ old('deskripsi', $kategoriPemasukan->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
