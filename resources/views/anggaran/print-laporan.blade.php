...
<body>
    <div class="header">
        ...
        <p>
            Periode:
            {{ $bulan ? \Carbon\Carbon::createFromFormat('m', $bulan)->format('F') . ' ' . $tahun : 'Tahun ' . $tahun }}
        </p>
    </div>
    ...
    <div class="summary">
        <div class="summary-grid">
            ...
            <div class="summary-item">
                <h3>Sisa Anggaran</h3>
                <div class="amount {{ $totalSisa >= 0 ? 'text-green' : 'text-red' }}">Rp
                    {{ number_format($totalSisa, 0, ',', '.') }}</div>
            </div>
        </div>
    </div>
                ...
                <tbody>
                    @foreach ($laporan as $item)
                        <tr>
                            ...
                            <td class="text-right text-red">{{ number_format($item->total_realisasi, 0, ',', '.') }}
                            </td>
    ...
    <div class="footer-info">
        ...
        @if ($totalRealisasi > $totalAnggaran)
            <p class="text-red font-bold">⚠️ Peringatan: Total pengeluaran melebihi anggaran sebesar Rp
                {{ number_format($totalRealisasi - $totalAnggaran, 0, ',', '.') }}</p>
        @endif
