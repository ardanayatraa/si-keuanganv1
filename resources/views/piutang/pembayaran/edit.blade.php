{{-- resources/views/piutang/pembayaran/edit.blade.php --}}
<x-app-layout>
    <div class="bg-white p-6 rounded-xl shadow-sm">
        <h1 class="text-2xl font-bold mb-4">Edit Pembayaran Piutang</h1>

        <form action="{{ route('piutang.pembayaran.update', $p->id_pembayaran_piutang) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                    ...
                    <select id="id_piutang" name="id_piutang"
                        ...
                        @foreach ($piutangs as $piutang)
                            ...
                            <option value="{{ $piutang->id_piutang }}" data-original="{{ $original }}"
                                ...
                                {{ old('id_piutang', $p->id_piutang) == $piutang->id_piutang ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="3"
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-yellow-500 focus:border-yellow-500">{{ old('deskripsi', $p->deskripsi) }}</textarea>
                </div>
            ...
            <div class="mt-6 flex justify-end">
                ...
                <button type="submit"
                    class="px-6 py-2 bg-yellow-600 text-white rounded-md shadow-sm hover:bg-yellow-700">Update</button>
            </div>
        </form>
    </div>
